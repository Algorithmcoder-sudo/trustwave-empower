...
const FeatureSection = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // We'll divide our section into 2 phases
      const scrollPosition = Math.max(0, -sectionTop + windowHeight * 0.1);
      
      // Calculate total scroll height
      const totalScrollHeight = section.clientHeight - windowHeight;
      
      // Phase height is 1/2 of the total scroll height
      const phaseHeight = totalScrollHeight / 2;
      const currentPhase = Math.min(1, Math.floor(scrollPosition / phaseHeight));
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        {isMobile ? (
          ...
          <div className="h-[180vh] relative">
            {/* Sticky container for the mobile device and labels - sticks to the viewport */}
            <div className="sticky top-20 h-[calc(100vh-10rem)] w-full flex items-center justify-center">
              ...
              <div ref={mobileDeviceRef} className="relative">
                ...
                {scrollPhase === 0 && scrollProgress < 0.5 && (
                  <>
            ...
            <div 
              className="absolute w-[380px] transition-all duration-700 ease-out"
              style={{
                opacity: scrollPhase === 0 && scrollProgress > 0.3 ? 
                  Math.min(1, (scrollProgress - 0.3) * 3) : 
                  scrollPhase >= 1 ? 1 : 0,
                right: '15%',
                top: scrollPhase === 0 ? 
                  `calc(50vh + ${((1 - scrollProgress) * 100) - 50}vh)` :
                  'calc(50vh - 240px)',
                visibility: (scrollPhase === 0 && scrollProgress > 0.3) || scrollPhase >= 1 ? 'visible' : 'hidden',
                transform: `translateY(${
                  scrollPhase === 0 ? 
                    Math.max(0, 100 - (scrollProgress - 0.3) * 200) :
                    0
                }px)`
              }}
            >
            ...
            <div 
              className="absolute w-[380px] transition-all duration-700 ease-out" 
              style={{
                opacity: scrollPhase >= 1 && scrollProgress > 0.2 ? 
                  Math.min(1, (scrollProgress - 0.2) * 3) : 0,
                left: '15%',
                top: `calc(50vh - 240px)`,
                visibility: scrollPhase >= 1 && scrollProgress > 0.2 ? 'visible' : 'hidden',
                transform: `translateY(${
                  scrollPhase >= 1 ? 
                    Math.max(0, 100 - (scrollProgress - 0.2) * 200) : 
                    100
                }px)`
              }}
            >
